...
export function useMediaMoreInfoModalMoreLikeThis(mediaInfo: MediaInfo) {
  const firstCategoryIndex = useMemo(
    () => Math.floor(Math.random() * MOCK_MAP_MAIN_CATEGORIES.size + 1),
    [],
  )
  ...
  const { isLoading: isFirstLoading, medias: firstMedias } =
    useRequestMainCategoryMedias(firstCategoryIndex.toString())

  const { isLoading: isSecondLoading, medias: secondMedias } =
    useRequestMainCategoryMedias(secondCategoryIndex.toString())

  const mergedMedias = [...(firstMedias || []), ...(secondMedias || [])].filter(
    ({ id }) => id !== mediaInfo.id,
  )
